...
## Attendance workspace integration — 2026-08-03

Web self-service is now embedded in the existing `/hr/attendance` workspace; no `/hr/my-attendance` route, redirect, sidebar item, or route-specific guard was created. `SelfAttendancePage` was converted into the route-free `SelfAttendanceSection`, preserving the fresh location, preflight, live camera, preview/retake, evidence upload, final check-in, check-out, refresh, and duplicate-request protections.

The workspace evaluates the authenticated session once: an active linked `HrEmployee` receives My Attendance; attendance managers receive Team Attendance; linked managers receive both with My Attendance selected by default; unlinked managers keep the team tools only. An unlinked user with neither eligible self-service nor management access receives a clear access state rather than a not-found response. A tab bar is rendered only when both sections are available. Team Attendance remains the sole owner of `POST /hr/attendance` manual creation/editing and review data.

Added role/default-tab coverage for employee, linked manager, unlinked manager, and no-section states. Validation passed: `pnpm --filter admin-web test` (6 files, 21 tests) and `pnpm --filter admin-web build`. No API source, database, deployment, commit, or push was changed for this integration. Remaining manual E2E: open `/hr/attendance` as each of the four account types, verify the responsive tab layout, then complete a browser check-in and check-out as a linked employee.
...
`GET /hr/attendance/me/policy` now resolves the authenticated active employee and derives the organization exclusively from that employee. It requires only the existing `hr.attendance.self` capability and returns a deliberately safe projection: web/mobile availability, location/photo/Wi-Fi requirements, web Wi-Fi behavior, GPS accuracy threshold, location freshness, `canCheckIn`, and blocking reason codes. It never accepts employee or organization IDs and excludes SSID/BSSID lists and all other administrative policy fields. The administrative settings endpoint remains unchanged for HR/Admin use.

`SelfAttendanceSection` now calls only the self-service policy endpoint. Explicit states cover policy loading, a linked employee with an eligible location (Check in), an open record (Check out), web-disabled/Wi-Fi-blocked policies, expired sessions, missing employee links, and unexpected failures with a request ID. Query retries are limited to transient transport/gateway failures; authorization failures are never retried. The self-service policy request also skips the API client's token-refresh retry so a 401 is reported as an expired session immediately. API diagnostics are development-only and deduplicated by status/method/path/message; attendance debug output remains development-only, state-change-only, and contains no tokens, GPS coordinates, or photo data. No data, migration, commit, push, or deployment was performed.

The self-service branch control had used `GET /hr/branches`, which returns `OrganizationBranch` administration records and does not establish an attendance geofence. The blank selection therefore did not identify a valid attendance location and the Check in action had no permitted target. The control now uses `GET /hr/attendance/me/locations`, a self-service endpoint that resolves the current active employee and returns only active `OrganizationAttendanceLocation` records in the current organization with `allowedForWeb`, a linked active branch, latitude, longitude, and a positive exact radius. The response explicitly exposes `branchId` and `radiusMeters`; an organization branch with no linked Attendance Location is never treated as eligible.

One eligible location is selected automatically; multiple eligible locations form the dropdown. The self-service preflight, final check-in, and check-out send both `attendanceLocationId` and the associated `branchId`. Final server verification constrains a supplied location ID to the current organization, active/channel-eligible locations, and the selected active branch. While locations load the section displays loading; no eligible location produces a clear setup message instead of silently hiding Check in, while an open record continues to expose Check out.

Added API coverage for the eligible-location contract and admin-web coverage for automatic selection, multiple locations, inactive/web-ineligible records, missing branch/geofence data, and empty setup. Validation passed: focused API attendance service tests (36), API build, admin-web tests (7 files, 28 tests), and admin-web build. No attendance records/attempts, database state, commit, push, or deployment were changed. Remaining E2E: verify the configured staging branch appears automatically for a linked employee, then verify the setup message on an organization with branches but no Attendance Location.

## Staging attendance location linkage diagnosis — 2026-08-03

Read-only staging inspection found `capitalgateegy` branch `cms9ioyjk000a0mrys3bdzx67` active in organization `cmrt1o5o500050mnm6gszhm27`. Its Attendance Location is `cms9io9om00080mryjphk9w2u`, active and web/mobile-enabled with coordinates `30.035761, 31.46035` and exact/expanded radii `30/1000`, but `officeId` is `null`. All three active linked employees in that organization also had `officeId` and `branchId` set to `null`; no data was modified. The current web-locations endpoint relies on the current employee's organization, then requires the Attendance Location to have a linked active branch in the same organization. It does not match location and branch names, and it does not use employee office/branch as a fallback.

Attendance Location creation now requires an Office/Branch ID server-side and in the Platform form. Existing unlinked locations appear under a dedicated “unlinked branch” section where an administrator can select an actual active office and explicitly save that one location's `officeId`; there is no automatic name-based migration. The employee office and branch fields are now dropdowns populated from actual OrganizationBranch records, preserving real IDs rather than free text. Validation passed: focused API attendance tests (36), API build, admin-web tests (28), and admin-web build. No staging data update, migration, delete, commit, push, or deployment was performed.
