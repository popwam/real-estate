...
export function ConversationDetailView({ id }: { id: string }) {
  ...
  if (conversation.error) return <FeedbackState tone="error" title={t("adminSweep.conversation.could.not.be.loaded.4c82bc79")} description={conversation.error.message} />;
  if (!conversation.data) return <FeedbackState tone="error" title={t("adminSweep.conversation.is.unavailable.3e3d0c47")} />;
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="ui-card p-4 sm:p-5" aria-labelledby="conversation-overview-title"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-wide text-[var(--color-accent)]">{formatLabel(data.type)}</p><h2 id="conversation-overview-title" className="mt-2 text-lg font-semibold text-[var(--color-foreground)]">{t("adminSweep.conversation.overview.ec35b62c")}</h2></div>{leadHref ? <Link href={leadHref} className="ui-button ui-button-secondary">{t("adminSweep.open.lead.51135f94")}</Link> : null}</div><dl className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Fact icon={<Building2 className="h-4 w-4" aria-hidden="true" />} label="Project" value={data.project?.name ?? "Not linked"} /><Fact icon={<UserRound className="h-4 w-4" aria-hidden="true" />} label="Lead" value={data.crmLead?.client?.name ?? "Not linked"} /><Fact icon={<CalendarDays className="h-4 w-4" aria-hidden="true" />} label="Created" value={formatDate(data.createdAt)} /><Fact icon={<MessageSquareText className="h-4 w-4" aria-hidden="true" />} label="Last updated" value={formatDate(data.updatedAt)} /></dl>{data.statusNote ? <p className="mt-4 border-t border-[var(--color-border)] pt-4 text-sm text-[var(--color-muted)]">{t("adminSweep.status.note.97cd7f1f")}{data.statusNote}</p> : null}</section>

      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="min-w-0 space-y-6">
          <section className="ui-card overflow-hidden" aria-labelledby="messages-title"><div className="border-b border-[var(--color-border)] px-4 py-3 sm:px-5"><h2 id="messages-title" className="text-sm font-semibold text-[var(--color-foreground)]">{t("adminSweep.messages.f1702b46")}</h2><p className="mt-1 text-xs text-[var(--color-muted)]">{t("adminSweep.no.unread.state.is.inferred.from.this.message.li.aa755ff3")}</p></div><div className="max-h-[62vh] min-h-80 overflow-y-auto bg-[var(--color-surface-muted)] p-4 sm:p-5">{messages.isLoading ? <LoadingState label="Loading messages" /> : null}{messages.error ? <FeedbackState tone="error" title={t("adminSweep.messages.could.not.be.loaded.257240eb")} description={messages.error.message} /> : null}{!messages.isLoading && !messages.error ? <ConversationMessagesTimeline messages={messages.data ?? []} /> : null}</div><div className="sticky bottom-0 border-t border-[var(--color-border)] bg-[var(--color-surface)] p-4 sm:p-5"><ConversationMessageComposer error={createMessage.error} isPending={createMessage.isPending} onSubmit={(input) => createMessage.mutateAsync(input)} /></div></section>

          <DetailCard title={t("adminSweep.conversation.activity.3d97b368")}><CrmActivityTimeline activities={activities.data?.items} error={activities.error} isLoading={activities.isLoading} pagination={activities.data?.pagination} onPageChange={(page) => setActivityQuery((current) => ({ ...current, page }))} onPageSizeChange={(pageSize) => setActivityQuery((current) => ({ ...current, page: 1, pageSize }))} /></DetailCard>
        </div>

        <aside className="space-y-5 xl:sticky xl:top-[calc(var(--topbar-height)+1.5rem)]" aria-label={t("adminSweep.conversation.context.and.actions.c9eb6e18")}>
          <DetailCard title={t("adminSweep.status.bae7d5be")}><ConversationStatusUpdateDialog conversationId={data.id} currentStatus={data.status} /></DetailCard>
          <DetailCard title={t("adminSweep.private.share.link.ada5af98")}><ConversationShareLinkBox shareToken={data.shareToken} /></DetailCard>
          <DetailCard title={t("adminSweep.participants.cd56e083")}><div className="space-y-2">{(data.participants ?? []).length ? data.participants?.map((participant) => <div key={participant.id} className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-3"><p className="text-sm font-semibold text-[var(--color-foreground)]">{participant.displayName || formatLabel(participant.publicRole)}</p><p className="mt-1 text-xs text-[var(--color-muted)]">{formatLabel(participant.publicRole)}{participant.joinedAt ? ` · Joined ${formatDate(participant.joinedAt)}` : ""}</p></div>) : <p className="rounded-[var(--radius-md)] border border-dashed border-[var(--color-border)] p-3 text-sm text-[var(--color-muted)]">{t("adminSweep.no.participant.details.were.returned.0b003f0e")}</p>}</div></DetailCard>
          <DetailCard title={t("adminSweep.lead.context.44cdb42a")}><div className="space-y-3"><ContextField label="Lead status" value={data.crmLead?.status ? formatLabel(data.crmLead.status) : "Not linked"} /><ContextField label="Preferred contact" value={formatLabel(data.crmLead?.preferredContactMethod ?? "NOT_SET")} /><ContextField label="Project" value={data.project?.name ?? "Not linked"} /><ContextField label="Organization" value={data.organization?.name ?? "Not returned"} /></div></DetailCard>
        </aside>
      </div>
    </div>
  );
