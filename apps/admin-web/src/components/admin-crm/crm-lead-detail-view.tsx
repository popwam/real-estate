...
export function CrmLeadDetailView({ id, conversationsBasePath, showClaimAction = false }: { id: string; conversationsBasePath: string; showClaimAction?: boolean }) {
  ...
  if (error) return <FeedbackState tone="error" title={t("adminSweep.crm.lead.could.not.be.loaded.333f41b9")} description={error.message} />;
  if (!lead) return <FeedbackState tone="error" title={t("adminSweep.crm.lead.is.unavailable.190e843c")} />;
  ...
  return (
    <div className="space-y-6">
      ...
      {createConversation.error ? <FeedbackState tone="error" title={t("adminSweep.conversation.could.not.be.opened.b856da67")} description={createConversation.error.message} /> : null}

      <section className="ui-card p-5" aria-labelledby="lead-overview-title">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--color-accent)]">{lead.client?.source ?? "CRM lead"}</p><h2 id="lead-overview-title" className="mt-2 text-xl font-semibold text-[var(--color-foreground)]">{t("adminSweep.lead.overview.599b9213")}</h2><p className="mt-2 text-sm leading-6 text-[var(--color-muted)]">{t("adminSweep.created.accf40c8")}{formatDate(lead.createdAt)}{t("adminSweep.updated.3874ded4")}{formatDate(lead.updatedAt)}</p></div>
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="min-w-0 space-y-6">
          <DetailCard title={t("adminSweep.project.and.attribution.2e4ae84f")}>
          ...
          <DetailCard title={t("adminSweep.visitor.behavior.b384380e")}>
            <VisitorBehavior lead={lead} />
          </DetailCard>

          <DetailCard title={t("adminSweep.pipeline.notes.and.follow.up.f72add88")}>
        ...
        <aside className="space-y-5 xl:sticky xl:top-[calc(var(--topbar-height)+1.5rem)]" aria-label={t("adminSweep.lead.actions.and.contact.5bc20917")}>
          <DetailCard title={t("adminSweep.contact.b37456c4")}>
          ...
          <DetailCard title={t("adminSweep.next.action.d816c3c7")}>
            <p className="text-sm leading-6 text-[var(--color-muted)]">{t("adminSweep.continue.the.conversation.before.moving.to.reser.69f63684")}</p>
            ...
            <Link href={conversationsBasePath} className="ui-button ui-button-secondary mt-2 w-full">{t("adminSweep.conversation.inbox.7e45354e")}<ExternalLink className="h-4 w-4" aria-hidden="true" /></Link>
          </DetailCard>

          <DetailCard title={t("adminSweep.lead.status.20dd0f14")}><CrmLeadStatusUpdateDialog leadId={lead.id} currentStatus={lead.status} /></DetailCard>
          <DetailCard title={t("adminSweep.ownership.3e90e4cb")}><BusinessField label="Claimed broker" value={lead.claimedByBroker ? brokerName(lead.claimedByBroker) : lead.claimedByBrokerUserId ? "Assigned broker" : "Unclaimed"} /><div className="mt-3"><BusinessField label="Claimed organization" value={lead.claimedByOrganization?.name ?? (lead.claimedByOrganizationId ? "Assigned organization" : "Unclaimed")} /></div></DetailCard>
        </aside>
      </div>
    </div>
  );
...
function AttributionSummary({ utm }: { utm?: Record<string, unknown> | null }) {
  ...
  if (!entries.length) return <p className="mt-5 rounded-[var(--radius-md)] border border-dashed border-[var(--color-border)] p-4 text-sm text-[var(--color-muted)]">{t("adminSweep.no.campaign.attribution.values.are.attached.f809d2ea")}</p>;
  return <div className="mt-5 border-t border-[var(--color-border)] pt-5"><h3 className="text-sm font-semibold text-[var(--color-foreground)]">{t("adminSweep.campaign.attribution.360ab01c")}</h3><dl className="mt-3 grid gap-3 sm:grid-cols-2">{entries.map(([key, value]) => <BusinessField key={key} label={formatLabel(key)} value={String(value)} />)}</dl></div>;
}
