...
export function ConversationStatusUpdateDialog({ conversationId, currentStatus }: { conversationId: string; currentStatus: ConversationStatus }) {
  ...
  return <div className="space-y-4"><label className="grid gap-2 text-xs font-semibold text-[var(--color-muted)]">{t("adminSweep.status.bae7d5be")}<select className="ui-input" value={status} onChange={(event) => { setStatus(event.target.value as ConversationStatus); update.reset(); }}>{statuses.map((item) => <option key={item} value={item}>{formatLabel(item)}</option>)}</select></label><label className="grid gap-2 text-xs font-semibold text-[var(--color-muted)]">{t("adminSweep.status.note.dc2f8f3c")}<Textarea value={statusNote} onChange={(event) => setStatusNote(event.target.value)} placeholder={t("adminSweep.optional.reason.or.handoff.note.49b6fb01")} /></label>{update.error ? <FeedbackState tone="error" title={t("adminSweep.conversation.status.could.not.be.updated.05e9772a")} description={update.error.message} /> : null}{update.isSuccess ? <FeedbackState tone="success" title={t("adminSweep.conversation.status.updated.5138480f")} /> : null}<Button className="w-full" disabled={update.isPending || (status === currentStatus && !statusNote.trim())} onClick={submit}>{update.isPending ? <LoaderCircle className="h-4 w-4 animate-spin" aria-hidden="true" /> : <Save className="h-4 w-4" aria-hidden="true" />}{update.isPending ? "Updating…" : "Update status"}</Button></div>;
}
