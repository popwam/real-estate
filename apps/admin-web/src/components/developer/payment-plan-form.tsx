  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit((value) => onSubmit({
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Field label="Plan name" error={errors.name?.message} required><TextInput placeholder="Example: 10% over 8 years" {...register("name")} /></Field>
        <Field label="Plan scope" required hint={scope === "PROJECT" ? "Available at project level." : "Targets one existing unit ID."}><SelectInput {...register("scope")}><option value="PROJECT">Project level</option><option value="UNIT">Unit level</option></SelectInput></Field>
      ...
      {error ? <FeedbackState tone="error" title="Payment plan could not be created" description={error.message} /> : null}
