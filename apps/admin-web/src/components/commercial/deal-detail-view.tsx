...
export function DealDetailView({ id }: { id: string }) {
  ...
  if (error) return <FeedbackState tone="error" title={t("adminSweep.deal.could.not.be.loaded.35e9197b")} description={error.message} />;
  if (!deal) return null;
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<>{deal.status === "PENDING_APPROVAL" ? <DealActionDialog action="approve" isPending={approve.isPending} error={approve.error} trigger={<Button>{t("adminSweep.approve.7b2c7f14")}</Button>} onConfirm={() => approve.mutateAsync(id)} /> : null}{deal.status !== "SOLD" && deal.status !== "CANCELLED" ? <DealActionDialog action="cancel" isPending={cancel.isPending} error={cancel.error} trigger={<Button className="bg-[var(--color-danger)] text-white hover:opacity-90">{t("adminSweep.cancel.deal.56a865a1")}</Button>} onConfirm={(input) => cancel.mutateAsync({ id, input })} /> : null}</>}
      />
      <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
        <DealSummaryCard deal={deal} />
        <div className="space-y-6">
        <DetailCard title={t("adminSweep.deal.progress.d85dcecc")}><DealStatusTimeline deal={deal} /></DetailCard>
        <DetailCard title={t("adminSweep.commission.entries.e087e2b8")}>
          <div className="space-y-2 text-sm text-[var(--color-text)]">
            ...
            )) : <p>{t("adminSweep.no.commission.entries.returned.for.this.deal.3e1dfa58")}</p>}
          </div>
        </DetailCard></div>
      </div>
    </>
