...
export function TeamAttendanceSection({
  ...
  async function exportAttendance() {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `attendance-${selectedDate.slice(0, 7)}.csv`;
      anchor.click();
  ...
  return (
    <OperationsPage
      ...
      filterPrefix={
        <div className="grid gap-2 text-sm md:col-span-2">
          <span className="font-medium text-zinc-700">
            {t("attendance.admin.date")}
          </span>
          <div className="flex flex-wrap items-center gap-2">
            <InputDate
              value={selectedDate}
              onChange={(value) => isDateOnly(value) && setDate(value)}
            />
            <Button
              type="button"
            ...
            >
              {t("attendance.admin.previousDay")}
            </Button>
            <Button
              type="button"
              className="ui-button-secondary"
            ...
            {canExport ? (
              <Button
                ...
                disabled={exporting}
                title={t("attendance.admin.export")}
              >
      ...
      fields={[
        ...
        {
          name: "faceVerificationStatus",
          label: t("attendance.admin.faceStatus"),
          type: "select",
          options: [
            "NOT_REQUIRED",
            "PENDING",
        ...
        {
          name: "faceVerificationConfidence",
          label: t("attendance.admin.faceConfidence"),
          type: "number",
        },
        {
      ...
      columns={[
        ...
        {
          name: "entryChannel",
          label: t("attendance.admin.entryChannel"),
          render: (row) =>
            String(row.entryChannel ?? row.attendanceSource ?? "-"),
        },
        {
          name: "plannedCheckInAt",
          label: "Planned check-in",
          render: (row) => formatDateTime(row.plannedCheckInAt),
        },
        {
          name: "minutesLate",
          label: "Late by",
          render: (row) =>
            row.minutesLate == null ? "-" : `${row.minutesLate} min`,
        },
        {
          name: "lateAllowanceChargedMinutes",
          label: t("attendance.admin.allowanceCharged"),
          render: (row) => formatMinutes(row.lateAllowanceChargedMinutes),
        },
        {
          name: "lateAllowanceRemainingMinutes",
          label: t("attendance.admin.allowanceRemaining"),
          render: (row) => formatMinutes(row.lateAllowanceRemainingMinutes),
        },
        {
          name: "checkOutAt",
          label: t("attendance.admin.checkOutAt"),
        ...
        {
          name: "autoClosed",
          label: t("attendance.autoClosedBadge"),
          render: (row) =>
            row.autoClosed ? (
              <span className="rounded-full bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-900">
                {t("attendance.autoClosedBadge")}
              </span>
            ) : (
              "-"
            ),
        },
        {
          ...
          render: (row) => formatDateTime(row.autoClosedAt),
        },
        {
          name: "autoCloseReason",
          label: "Auto-close reason",
          render: (row) => String(row.autoCloseReason ?? "-"),
        },
        {
          name: "plannedCheckOutAt",
          label: "Planned check-out",
          render: (row) => formatDateTime(row.plannedCheckOutAt),
        },
        {
          name: "checkOutMethod",
          label: "Check-out method",
          render: (row) => String(row.checkOutMethod ?? "-"),
        },
        {
          name: "checkOutVerificationStatus",
          label: "Check-out location verification",
          render: (row) =>
            String(row.checkOutVerificationStatus ?? "NOT_VERIFIED"),
        },
        {
          name: "lastVerifiedLocation",
          label: "Last verified location",
          render: (row) => branchName(row),
        },
        {
          name: "requiresManualReview",
          label: "Requires review",
          render: (row) => (row.requiresManualReview ? "Yes" : "-"),
        },
        { name: "status", label: t("attendance.admin.status") },
        {
          name: "attendanceStatusAtCheckIn",
          label: "Check-in status",
          render: (row) =>
            String(row.attendanceStatusAtCheckIn ?? row.status ?? "-"),
        },
        {
          name: "scheduleSource",
          label: "Schedule source",
          render: (row) => String(row.scheduleSource ?? "-"),
        },
        {
          name: "scheduleTimezone",
          label: "Schedule timezone",
          render: (row) => String(row.scheduleTimezone ?? "-"),
        },
        {
          name: "lateUntilAt",
          label: "Late threshold",
          render: (row) => formatDateTime(row.lateUntilAt),
        },
        {
          name: "severeLateUntilAt",
          label: "Severe late threshold",
          render: (row) => formatDateTime(row.severeLateUntilAt),
        },
        {
          name: "absentAfterAt",
          label: "Absent threshold",
          render: (row) => formatDateTime(row.absentAfterAt),
        },
        {
          name: "verificationStatus",
          label: t("attendance.admin.verificationStatus"),
        ...
        },
        {
          ...
          label: t("attendance.admin.referenceImage"),
        },
...
function branchName(row: Record<string, unknown>) {
  const branch = row.branch;
  if (branch && typeof branch === "object" && "name" in branch) {
    const name = (branch as { name?: unknown }).name;
    if (typeof name === "string" && name.trim()) return name;
  }
  return "-";
}
...
function formatMinutes(value: unknown) {
  ...
  return Number.isFinite(minutes) ? `${minutes / 60} h` : "-";
}
