          ...
          { key: "verifiedAt", header: "Verified", cell: (row) => formatDate(row.verifiedAt) },
          {
            ...
            cell: (row) => (
              <div className="flex flex-wrap gap-2">
                <Button className="h-8 bg-white px-2 text-zinc-700 ring-1 ring-inset ring-zinc-200 hover:bg-zinc-50" disabled={isWorking} onClick={() => onRequestVerification(row.id)}>
                  Request verification
                </Button>
                <Button className="h-8 bg-white px-2 text-blue-700 ring-1 ring-inset ring-blue-200 hover:bg-blue-50" disabled={isWorking} onClick={() => onCheckDns(row.id)}>
                  Check DNS
                </Button>
                {showDevActions ? (
                  <Button className="h-8 bg-white px-2 text-emerald-700 ring-1 ring-inset ring-emerald-200 hover:bg-emerald-50" disabled={isWorking} onClick={() => onMarkVerifiedDevOnly(row.id)}>
                    Dev verify
                  </Button>
                ) : null}
              </div>
