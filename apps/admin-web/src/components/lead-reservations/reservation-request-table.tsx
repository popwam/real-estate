...
export function ReservationRequestTable({ requests, basePath, mode, onApprove, onReject, onCancel, isWorking, error }: {
  ...
  if (!requests.length) {
    return <div className="ui-empty-state"><CalendarClock className="h-8 w-8" aria-hidden="true" /><h3>{t("adminSweep.no.reservation.requests.06a9515e")}</h3><p>{mode === "developer" ? "New broker requests will appear here for review." : "Requests you submit to developers will appear here."}</p></div>;
  }
        ...
        <article key={request.id} className="ui-card flex min-w-0 flex-col p-5">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--color-text-muted)]">{t("adminSweep.reservation.request.fb0a8809")}</p>
          ...
          {request.rejectionReason ? <div className="ui-feedback ui-feedback-error mt-4 text-sm"><strong>{t("adminSweep.developer.response.353e4994")}</strong> {request.rejectionReason}</div> : null}
          <div className="mt-auto flex flex-wrap items-center gap-2 pt-5">
            <Link className="ui-button ui-button-secondary" href={`${basePath}/${request.id}`}>{t("adminSweep.review.request.525f174f")}<ArrowUpRight className="h-4 w-4" aria-hidden="true" /></Link>
            {mode === "developer" && request.status === "PENDING" && onApprove ? <ReservationActionDialog action="approve" isPending={isWorking} error={error} trigger={<Button>{t("adminSweep.approve.7b2c7f14")}</Button>} onConfirm={() => onApprove(request.id)} /> : null}
            {mode === "developer" && request.status === "PENDING" && onReject ? <ReservationActionDialog action="reject" isPending={isWorking} error={error} trigger={<Button className="bg-[var(--color-danger)] text-white hover:opacity-90">{t("adminSweep.reject.2b03b592")}</Button>} onConfirm={(input) => onReject(request.id, input.reason ?? "")} /> : null}
            {mode === "brokerage" && request.status === "PENDING" && onCancel ? <ReservationActionDialog action="cancel" isPending={isWorking} error={error} trigger={<Button className="ui-button-secondary">{t("adminSweep.cancel.77dfd213")}</Button>} onConfirm={() => onCancel(request.id)} /> : null}
          </div>
        </article>
      ))}
    </div>
