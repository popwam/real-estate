...
export default function ProjectPaymentPlansPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={`${project.data?.name ?? "Project"} payment plans`} description="Keep project-wide offers separate from plans that target a specific unit." actions={<div className="flex flex-wrap gap-2"><Link href={`/developer/projects/${id}`} className="ui-button ui-button-secondary"><ArrowLeft className="h-4 w-4" aria-hidden="true" />Project overview</Link><button type="button" className="ui-button ui-button-primary" onClick={() => setFormOpen((open) => !open)}><Plus className="h-4 w-4" aria-hidden="true" />New plan</button></div>} />

      <div className="grid gap-4 sm:grid-cols-2">
        <ScopeCard title="Project-level plan" description="Applies as a general commercial option for the project and does not require a unit ID." />
        <ScopeCard title="Unit-level plan" description="Targets one existing unit. The unit ID remains part of the existing payment-plan payload." />
      </div>

      {formOpen ? <DetailCard title="Create payment plan"><PaymentPlanForm key={formVersion} isPending={create.isPending} error={create.error} onSubmit={async (input) => { await create.mutateAsync(input); setFormVersion((value) => value + 1); setFormOpen(false); }} /></DetailCard> : null}

      {plans.isLoading ? <LoadingState label="Loading payment plans" /> : null}
      {plans.error ? <FeedbackState tone="error" title="Payment plans could not be loaded" description={plans.error.message} /> : null}
      {create.isSuccess && !formOpen ? <FeedbackState tone="success" title="Payment plan created" /> : null}
      {!plans.isLoading && !plans.error ? (
        <section aria-labelledby="payment-plans-title">
          <div className="mb-4"><h2 id="payment-plans-title" className="text-lg font-semibold text-[var(--color-foreground)]">Saved plans</h2><p className="mt-1 text-sm text-[var(--color-muted)]">{(plans.data ?? []).length.toLocaleString()} {(plans.data ?? []).length === 1 ? "plan" : "plans"}</p></div>
          {(plans.data ?? []).length ? (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {(plans.data ?? []).map((plan) => (
                <article key={plan.id} className="ui-card p-5">
                  ...
                  {plan.scope === "UNIT" ? <p className="mt-3 text-xs text-[var(--color-muted)]">Unit: {plan.unit?.unitNumber ?? plan.unitId ?? "Not specified"}</p> : null}
          ...
          ) : <EmptyState icon={<CreditCard className="h-5 w-5" aria-hidden="true" />} title="No payment plans yet" description="Create a project-level plan or target a supported unit using the existing payment-plan fields." action={<button type="button" className="ui-button ui-button-primary" onClick={() => setFormOpen(true)}><Plus className="h-4 w-4" aria-hidden="true" />Create first plan</button>} />}
        </section>
      ) : null}
    </div>
  );
