import { PrismaPg } from '@prisma/adapter-pg';
import { PrismaClient } from '@prisma/client';
import { loadEnvironment } from '../../config/load-environment';

export const BASE_ROLES = [
  'platform_owner',
  'platform_admin',
  'platform_support',
  'platform_auditor',
  'platform_hr',
  'platform_sales',
  'platform_admin_limited',
  'developer_owner',
  'developer_admin',
  'developer_sales_manager',
  'developer_sales_agent',
  'brokerage_owner',
  'brokerage_admin',
  'broker',
  'individual_broker',
  'client',
  'company_admin',
  'hr_manager',
  'hr_employee',
  'sales_manager',
  'sales_agent',
  'finance_user',
  'employee_self_service',
] as const;

export const GLOBAL_HR_PERMISSIONS = [
  'hr.dashboard.view',
  'hr.recruitment.view',
  'hr.recruitment.manage',
  'hr.recruitment.applicants.view',
  'hr.recruitment.applicants.manage',
  'hr.recruitment.documents.view',
  'hr.recruitment.documents.manage',
  'hr.recruitment.interviews.manage',
  'hr.recruitment.offers.manage',
  'hr.recruitment.convert_to_employee',
  'hr.employees.delete',
  'hr.work_groups.view',
  'hr.work_groups.manage',
  'hr.teams.view',
  'hr.teams.manage',
  'hr.actions.view',
  'hr.actions.apply',
  'hr.documents.view',
  'hr.documents.manage',
  'hr.org_chart.view',
  'hr.transfer_log.view',
  'hr.title_changes.view',
  'hr.requests.view',
  'hr.requests.manage',
  'hr.attendance.manage',
  'hr.finance.view',
  'hr.finance.manage',
  'hr.assets.view',
  'hr.assets.manage',
  'hr.tasks.view',
  'hr.tasks.manage',
  'hr.hr_documents.view',
  'hr.hr_documents.manage',
  'hr.reports.view',
  'hr.reports.export',
  'hr.settings.view',
  'hr.settings.manage',
] as const;

export const HR_RECRUITMENT_PERMISSIONS = [
  'hr.recruitment.view',
  'hr.recruitment.manage',
  'hr.recruitment.applicants.view',
  'hr.recruitment.applicants.manage',
  'hr.recruitment.documents.view',
  'hr.recruitment.documents.manage',
  'hr.recruitment.interviews.manage',
  'hr.recruitment.offers.manage',
  'hr.recruitment.convert_to_employee',
  'hr.applicants.create',
  'hr.applicants.review',
  'hr.interviews.view',
  'hr.interviews.manage',
] as const;

export const RESIDENT_QR_PERMISSIONS = [
  'customers.view',
  'customers.manage',
  'real_estate.projects.view',
  'real_estate.projects.manage',
  'real_estate.buildings.view',
  'real_estate.buildings.manage',
  'real_estate.units.view',
  'real_estate.units.manage',
  'real_estate.unit_assignments.view',
  'real_estate.unit_assignments.manage',
  'qr_passes.view',
  'qr_passes.manage',
  'qr_passes.scan',
  'navigation.customize',
  'quick_actions.customize',
] as const;

export const SELF_RESIDENT_PERMISSIONS = [
  'self.units.view',
  'self.qr_passes.view',
] as const;

export const BASE_PERMISSIONS = [
  'platform.dashboard.view',
  'organizations.verify',
  'organizations.suspend',
  'organizations.view_all',
  'platform.organizations.view',
  'platform.organizations.create',
  'platform.organizations.update',
  'platform.organizations.archive',
  'platform.organizations.restore',
  'platform.organizations.delete_draft',
  'platform.organizations.purge',
  'platform.organizations.verify',
  'platform.organizations.manage',
  'platform.organizations.activate',
  'platform.organizations.suspend',
  'platform.verifications.view',
  'platform.verifications.manage',
  'platform.investigations.view',
  'platform.investigations.manage',
  'platform.documents.review',
  'platform.documents.view',
  'platform.subscriptions.manage',
  'platform.settings',
  'platform.settings.view',
  'platform.settings.manage',
  'platform.plans.view',
  'platform.plans.manage',
  'platform.subscriptions.view',
  'platform.metadata.view',
  'platform.metadata.manage',
  'platform.languages.view',
  'platform.languages.manage',
  'platform.countries.view',
  'platform.countries.manage',
  'platform.currencies.view',
  'platform.currencies.manage',
  'platform.exchange_rates.view',
  'platform.exchange_rates.manage',
  'platform.organization_types.view',
  'platform.organization_types.manage',
  'platform.organization_onboarding.view',
  'platform.organization_onboarding.manage',
  'platform.documents.manage',
  'platform.units.view',
  'platform.units.manage',
  'platform.integrations.view',
  'platform.integrations.manage',
  'platform.navigation.view',
  'platform.navigation.manage',
  'platform.verification_policies.view',
  'platform.verification_policies.manage',
  'subscriptions.manage',
  'disputes.resolve',
  'audit_logs.view',
  'reports.platform_wide',
  'users.impersonate',
  'platform.users.manage',
  'organizations.update',
  'organizations.update_own',
  'users.manage',
  'users.manage_own_org',
  'projects.create',
  'projects.publish',
  'projects.edit',
  'inventory.create',
  'inventory.publish',
  'inventory.view_private',
  'commission_rules.manage',
  'broker_access.grant',
  'reservation_requests.approve',
  'deal_rooms.create',
  'deal_rooms.join',
  'deals.approve',
  'deals.mark_sold',
  'commissions.view',
  'commissions.approve',
  'brokerage_agreements.manage',
  'reports.developer',
  'marketplace.view',
  'inventory.view_approved',
  'lead_claims.create',
  'lead_claims.view_own',
  'deal_requests.create',
  'commissions.view_own',
  'organization_website.update_own',
  'organization_website.view_own',
  'organization_domains.manage_own',
  'organization_domains.view_own',
  'organization_domains.verify',
  'public_leads.view_own',
  'public_leads.manage_own',
  'public_leads.view_all',
  'public_leads.manage_all',
  'crm.leads.view_own',
  'crm.leads.manage_own',
  'company.dashboard.view',
  'company.settings.view',
  'company.settings.manage',
  'company.profile.view',
  'company.profile.manage',
  'company.domains.view',
  'company.domains.manage',
  'company.offices.view',
  'company.offices.manage',
  'company.access_levels.view',
  'company.access_levels.manage',
  'company.attendance_locations.view',
  'company.attendance_locations.manage',
  'company.attendance_settings.view',
  'company.attendance_settings.manage',
  'company.wifi_rules.view',
  'company.wifi_rules.manage',
  ...RESIDENT_QR_PERMISSIONS,
  ...SELF_RESIDENT_PERMISSIONS,
  'hr.employees.view',
  'hr.employees.manage',
  'crm.view',
  'crm.manage',
  'finance.view',
  'finance.manage',
  'legal.contracts.view',
  'legal.contracts.manage',
  'cameras.integrations.view',
  'cameras.integrations.manage',
  'advertising.integrations.view',
  'advertising.integrations.manage',
  'hr.employees.create',
  'hr.employees.update',
  'hr.employees.deactivate',
  'hr.employees.reset_password',
  'hr.employees.permissions.manage',
  ...GLOBAL_HR_PERMISSIONS,
  'hr.attendance.self',
  'hr.attendance.view',
  'hr.attendance.review',
  'hr.attendance.export',
  'crm.leads.view',
  'crm.leads.create',
  'crm.leads.update',
  'crm.leads.assign',
  'crm.leads.manage',
  'deals.view',
  'deals.create',
  'deals.update',
  'deals.manage',
  'deal_rooms.manage',
  'files.view',
  'files.upload',
  'reports.view',
  'reports.export',
  'crm.leads.claim',
  'crm.conversations.view_own',
  'crm.conversations.manage_own',
  'crm.conversations.view_project',
  'crm.clients.view_own',
  'crm.clients.manage_own',
  'crm.pipeline.view_own',
  'crm.pipeline.manage_own',
  'crm.notes.view_own',
  'crm.notes.manage_own',
  'crm.tasks.view_own',
  'crm.tasks.manage_own',
  'hr.view',
  'hr.manage',
  'accounting.view',
  'accounting.manage',
  'legal.view',
  'legal.manage',
  'ads.view',
  'ads.manage',
  'cameras.view',
  'cameras.manage',
  'imports.project_inventory',
  'imports.hr',
  'imports.accounting',
  'imports.legal',
  'imports.ads',
  'imports.cameras',
  'exports.organization_data',
  'exports.platform_data',
  'brokers.manage',
  'reports.brokerage',
  'reports.own',
  'disputes.raise',
] as const;

export const PLATFORM_PERMISSIONS = BASE_PERMISSIONS.filter((permission) =>
  permission.startsWith('platform.'),
);

export const ROLE_PERMISSIONS: Record<string, readonly string[]> = {
  platform_owner: [...new Set([...BASE_PERMISSIONS, ...PLATFORM_PERMISSIONS])],
  platform_admin: [
    'organizations.verify',
    'organizations.suspend',
    'organizations.view_all',
    'platform.organizations.view',
    'platform.organizations.manage',
    'platform.organizations.activate',
    'platform.documents.review',
    'platform.settings.view',
    'platform.settings.manage',
    'platform.plans.view',
    'platform.plans.manage',
    'platform.metadata.view',
    'platform.metadata.manage',
    'platform.languages.view',
    'platform.languages.manage',
    'platform.countries.view',
    'platform.countries.manage',
    'platform.currencies.view',
    'platform.currencies.manage',
    'platform.exchange_rates.view',
    'platform.exchange_rates.manage',
    'platform.organization_types.view',
    'platform.organization_types.manage',
    'platform.organization_onboarding.view',
    'platform.organization_onboarding.manage',
    'platform.documents.view',
    'platform.documents.manage',
    'platform.units.view',
    'platform.units.manage',
    'platform.subscriptions.view',
    'platform.subscriptions.manage',
    'platform.verification_policies.view',
    'platform.verification_policies.manage',
    'organizations.update',
    'subscriptions.manage',
    'disputes.resolve',
    'audit_logs.view',
    'reports.platform_wide',
    'users.manage',
    'platform.users.manage',
    ...RESIDENT_QR_PERMISSIONS,
    ...SELF_RESIDENT_PERMISSIONS,
    'organization_website.update_own',
    'organization_website.view_own',
    'organization_domains.manage_own',
    'organization_domains.view_own',
    'organization_domains.verify',
    'public_leads.view_own',
    'public_leads.manage_own',
    'public_leads.view_all',
    'public_leads.manage_all',
    'crm.leads.view_own',
    'crm.leads.manage_own',
    'crm.leads.claim',
    'crm.conversations.view_own',
    'crm.conversations.manage_own',
    'crm.conversations.view_project',
    'crm.clients.view_own',
    'crm.clients.manage_own',
    'crm.pipeline.view_own',
    'crm.pipeline.manage_own',
    'crm.notes.view_own',
    'crm.notes.manage_own',
    'crm.tasks.view_own',
    'crm.tasks.manage_own',
    'hr.view',
    'hr.manage',
    ...HR_RECRUITMENT_PERMISSIONS,
    'hr.employees.view',
    'hr.employees.manage',
    'hr.employees.create',
    'hr.employees.update',
    'hr.employees.deactivate',
    'hr.employees.reset_password',
    'hr.employees.permissions.manage',
    'hr.attendance.self',
    'hr.attendance.view',
    'hr.attendance.manage',
    'hr.attendance.export',
    'accounting.view',
    'accounting.manage',
    'legal.view',
    'legal.manage',
    'ads.view',
    'ads.manage',
    'cameras.view',
    'cameras.manage',
    'imports.project_inventory',
    'imports.hr',
    'imports.accounting',
    'imports.legal',
    'imports.ads',
    'imports.cameras',
    'deal_rooms.join',
    'exports.organization_data',
    'exports.platform_data',
  ],
  platform_support: [
    'organizations.view_all',
    'audit_logs.view',
    'disputes.resolve',
  ],
  platform_auditor: [
    'organizations.view_all',
    'audit_logs.view',
    'reports.platform_wide',
  ],
  platform_hr: [
    'organizations.view_all',
    'hr.view',
    'hr.manage',
    ...HR_RECRUITMENT_PERMISSIONS,
    'hr.employees.view',
    'hr.employees.manage',
    'hr.employees.create',
    'hr.employees.update',
    'hr.employees.deactivate',
    'hr.employees.reset_password',
    'hr.employees.permissions.manage',
    'hr.attendance.self',
    'hr.attendance.view',
    'hr.attendance.manage',
    'hr.attendance.export',
    'hr.attendance.review',
    'hr.attendance.export',
    'reports.view',
  ],
  platform_sales: [
    'organizations.view_all',
    'crm.leads.view',
    'crm.leads.create',
    'crm.leads.update',
    'crm.leads.assign',
    'deal_rooms.join',
    'deals.view',
    'reports.view',
  ],
  platform_admin_limited: [
    'organizations.view_all',
    'platform.organizations.view',
    'platform.users.manage',
    'hr.view',
    'hr.employees.view',
    'reports.view',
  ],
  developer_owner: [
    'organizations.update_own',
    'users.manage_own_org',
    ...RESIDENT_QR_PERMISSIONS,
    ...SELF_RESIDENT_PERMISSIONS,
    'organization_website.update_own',
    'organization_website.view_own',
    'organization_domains.manage_own',
    'organization_domains.view_own',
    'public_leads.view_own',
    'public_leads.manage_own',
    'crm.leads.view_own',
    'crm.leads.manage_own',
    'crm.conversations.view_own',
    'crm.conversations.manage_own',
    'crm.conversations.view_project',
    'crm.clients.view_own',
    'crm.clients.manage_own',
    'crm.pipeline.view_own',
    'crm.pipeline.manage_own',
    'crm.notes.view_own',
    'crm.notes.manage_own',
    'crm.tasks.view_own',
    'crm.tasks.manage_own',
    'hr.view',
    'hr.manage',
    ...HR_RECRUITMENT_PERMISSIONS,
    'hr.employees.view',
    'hr.employees.create',
    'hr.employees.update',
    'hr.employees.deactivate',
    'hr.employees.reset_password',
    'hr.employees.permissions.manage',
    'hr.attendance.self',
    'hr.attendance.view',
    'hr.attendance.manage',
    'hr.attendance.export',
    'accounting.view',
    'accounting.manage',
    'legal.view',
    'legal.manage',
    'ads.view',
    'ads.manage',
    'cameras.view',
    'cameras.manage',
    'imports.project_inventory',
    'imports.hr',
    'imports.accounting',
    'imports.legal',
    'imports.ads',
    'imports.cameras',
    'exports.organization_data',
    'projects.create',
    'projects.publish',
    'projects.edit',
    'inventory.create',
    'inventory.publish',
    'inventory.view_private',
    'commission_rules.manage',
    'broker_access.grant',
    'reservation_requests.approve',
    'deal_rooms.create',
    'deal_rooms.join',
    'deals.approve',
    'deals.mark_sold',
    'commissions.view',
    'commissions.approve',
    'brokerage_agreements.manage',
    'reports.developer',
  ],
  developer_admin: [
    'organizations.update_own',
    'users.manage_own_org',
    ...RESIDENT_QR_PERMISSIONS,
    ...SELF_RESIDENT_PERMISSIONS,
    'organization_website.update_own',
    'organization_website.view_own',
    'organization_domains.manage_own',
    'organization_domains.view_own',
    'public_leads.view_own',
    'public_leads.manage_own',
    'crm.leads.view_own',
    'crm.leads.manage_own',
    'crm.conversations.view_own',
    'crm.conversations.manage_own',
    'crm.conversations.view_project',
    'crm.clients.view_own',
    'crm.clients.manage_own',
    'hr.view',
    'hr.manage',
    ...HR_RECRUITMENT_PERMISSIONS,
    'hr.employees.view',
    'hr.employees.create',
    'hr.employees.update',
    'hr.employees.deactivate',
    'hr.employees.reset_password',
    'hr.employees.permissions.manage',
    'hr.attendance.self',
    'hr.attendance.view',
    'hr.attendance.manage',
    'hr.attendance.export',
    'hr.attendance.review',
    'imports.project_inventory',
    'imports.hr',
    'imports.accounting',
    'imports.legal',
    'imports.ads',
    'imports.cameras',
    'exports.organization_data',
    'projects.create',
    'projects.publish',
    'projects.edit',
    'inventory.create',
    'inventory.publish',
    'inventory.view_private',
    'broker_access.grant',
    'reservation_requests.approve',
    'deal_rooms.create',
    'deal_rooms.join',
    'deals.approve',
    'commissions.view',
    'reports.developer',
    'crm.leads.view_own',
    'crm.conversations.view_project',
    'crm.clients.view_own',
    'exports.organization_data',
  ],
  developer_sales_manager: [
    'projects.edit',
    'inventory.view_private',
    'reservation_requests.approve',
    'deal_rooms.create',
    'deal_rooms.join',
    'deals.approve',
    'deals.mark_sold',
    'commissions.view',
    'reports.developer',
  ],
  developer_sales_agent: [
    'inventory.view_private',
    'deal_rooms.join',
    'reports.own',
    'exports.organization_data',
    'crm.leads.view_own',
    'crm.conversations.view_project',
    'crm.clients.view_own',
  ],
  brokerage_owner: [
    'organizations.update_own',
    'users.manage_own_org',
    ...RESIDENT_QR_PERMISSIONS,
    ...SELF_RESIDENT_PERMISSIONS,
    'organization_website.view_own',
    'organization_domains.view_own',
    'organization_domains.manage_own',
    'public_leads.view_own',
    'public_leads.manage_own',
    'crm.leads.view_own',
    'crm.leads.manage_own',
    'crm.leads.claim',
    'crm.conversations.view_own',
    'crm.conversations.manage_own',
    'crm.clients.view_own',
    'crm.clients.manage_own',
    'crm.pipeline.view_own',
    'crm.pipeline.manage_own',
    'crm.notes.view_own',
    'crm.notes.manage_own',
    'crm.tasks.view_own',
    'crm.tasks.manage_own',
    'hr.view',
    'hr.manage',
    ...HR_RECRUITMENT_PERMISSIONS,
    'hr.employees.view',
    'hr.employees.create',
    'hr.employees.update',
    'hr.employees.deactivate',
    'hr.employees.reset_password',
    'hr.employees.permissions.manage',
    'hr.attendance.self',
    'hr.attendance.view',
    'hr.attendance.manage',
    'hr.attendance.export',
    'hr.attendance.review',
    'exports.organization_data',
    'marketplace.view',
    'inventory.view_approved',
    'lead_claims.create',
    'lead_claims.view_own',
    'deal_requests.create',
    'deal_rooms.join',
    'commissions.view_own',
    'brokers.manage',
    'reports.brokerage',
    'disputes.raise',
  ],
  brokerage_admin: [
    'organizations.update_own',
    'users.manage_own_org',
    ...RESIDENT_QR_PERMISSIONS,
    ...SELF_RESIDENT_PERMISSIONS,
    'organization_website.view_own',
    'organization_domains.view_own',
    'organization_domains.manage_own',
    'public_leads.view_own',
    'public_leads.manage_own',
    'crm.leads.view_own',
    'crm.leads.manage_own',
    'crm.leads.claim',
    'crm.conversations.view_own',
    'crm.conversations.manage_own',
    'crm.clients.view_own',
    'crm.clients.manage_own',
    'crm.pipeline.view_own',
    'crm.pipeline.manage_own',
    'crm.notes.view_own',
    'crm.notes.manage_own',
    'crm.tasks.view_own',
    'crm.tasks.manage_own',
    'hr.view',
    'hr.manage',
    ...HR_RECRUITMENT_PERMISSIONS,
    'hr.employees.view',
    'hr.employees.create',
    'hr.employees.update',
    'hr.employees.deactivate',
    'hr.employees.reset_password',
    'hr.employees.permissions.manage',
    'hr.attendance.self',
    'hr.attendance.view',
    'hr.attendance.manage',
    'hr.attendance.export',
    'hr.attendance.review',
    'exports.organization_data',
    'marketplace.view',
    'inventory.view_approved',
    'lead_claims.view_own',
    'deal_rooms.join',
    'commissions.view_own',
    'brokers.manage',
    'reports.brokerage',
    'disputes.raise',
  ],
  broker: [
    'marketplace.view',
    'inventory.view_approved',
    'lead_claims.create',
    'lead_claims.view_own',
    'deal_requests.create',
    'deal_rooms.join',
    'commissions.view_own',
    'reports.own',
    'disputes.raise',
    'crm.leads.view_own',
    'crm.leads.claim',
    'crm.conversations.view_own',
    'crm.conversations.manage_own',
    'crm.clients.view_own',
  ],
  individual_broker: [
    'marketplace.view',
    'inventory.view_approved',
    'lead_claims.create',
    'lead_claims.view_own',
    'deal_requests.create',
    'deal_rooms.join',
    'commissions.view_own',
    'reports.own',
    'disputes.raise',
    'crm.leads.view_own',
    'crm.leads.claim',
    'crm.conversations.view_own',
    'crm.conversations.manage_own',
    'crm.clients.view_own',
  ],
  client: ['deal_rooms.join', ...SELF_RESIDENT_PERMISSIONS],
  company_admin: [
    'company.dashboard.view',
    'company.settings.view',
    'company.settings.manage',
    'company.profile.view',
    'company.profile.manage',
    'company.domains.view',
    'company.domains.manage',
    'company.offices.view',
    'company.offices.manage',
    'company.attendance_locations.view',
    'company.attendance_locations.manage',
    'company.attendance_settings.view',
    'company.attendance_settings.manage',
    'company.wifi_rules.view',
    'company.wifi_rules.manage',
    'company.access_levels.view',
    'company.access_levels.manage',
    'users.manage_own_org',
    'organizations.update_own',
    ...RESIDENT_QR_PERMISSIONS,
    ...SELF_RESIDENT_PERMISSIONS,
    'hr.view',
    'hr.manage',
    ...HR_RECRUITMENT_PERMISSIONS,
    'hr.employees.view',
    'hr.employees.manage',
    'hr.employees.create',
    'hr.employees.update',
    'hr.employees.deactivate',
    'hr.employees.reset_password',
    'hr.employees.permissions.manage',
    'hr.attendance.self',
    'hr.attendance.view',
    'hr.attendance.manage',
    'hr.attendance.review',
    'crm.leads.view_own',
    'crm.leads.manage_own',
    'crm.leads.view',
    'crm.leads.create',
    'crm.leads.update',
    'crm.leads.assign',
    'crm.leads.manage',
    'deal_rooms.create',
    'deal_rooms.join',
    'deal_rooms.manage',
    'deals.view',
    'deals.create',
    'deals.update',
    'deals.manage',
    'files.view',
    'files.upload',
    'reports.view',
    'reports.export',
    'exports.organization_data',
    'organization_website.view_own',
    'organization_website.update_own',
  ],
  hr_manager: [
    'company.dashboard.view',
    'company.profile.view',
    'company.attendance_settings.view',
    'company.wifi_rules.view',
    'hr.view',
    'hr.manage',
    ...HR_RECRUITMENT_PERMISSIONS,
    'hr.employees.view',
    'hr.employees.manage',
    'hr.employees.create',
    'hr.employees.update',
    'hr.employees.deactivate',
    'hr.employees.reset_password',
    'hr.employees.permissions.manage',
    'hr.attendance.self',
    'hr.attendance.view',
    'hr.attendance.manage',
    'hr.attendance.review',
    'files.view',
    'reports.view',
  ],
  hr_employee: [
    'company.dashboard.view',
    'hr.attendance.self',
    'files.view',
  ],
  sales_manager: [
    'company.dashboard.view',
    'crm.leads.view_own',
    'crm.leads.manage_own',
    'crm.leads.view',
    'crm.leads.create',
    'crm.leads.update',
    'crm.leads.assign',
    'crm.leads.manage',
    'deal_rooms.create',
    'deal_rooms.join',
    'deal_rooms.manage',
    'deals.view',
    'deals.create',
    'deals.update',
    'deals.manage',
    'reports.view',
  ],
  sales_agent: [
    'company.dashboard.view',
    'crm.leads.view_own',
    'crm.leads.claim',
    'deal_rooms.join',
    'deals.view',
    'reports.own',
  ],
  finance_user: [
    'company.dashboard.view',
    'accounting.view',
    'reports.view',
    'reports.export',
    'commissions.view',
  ],
  employee_self_service: [
    'company.dashboard.view',
    'hr.attendance.self',
  ],
};

export async function seedBaseRolesAndPermissions(prisma: PrismaClient) {
  let permissionsSeeded = 0;
  let rolesSeeded = 0;

  for (const key of BASE_PERMISSIONS) {
    await prisma.permission.upsert({
      where: { key },
      create: { key, description: `Base permission: ${key}` },
      update: {},
    });
    permissionsSeeded += 1;
  }

  for (const name of BASE_ROLES) {
    const existingRole = await prisma.role.findFirst({
      where: { organizationId: null, name },
    });
    const systemRole = existingRole
      ? await prisma.role.update({
          where: { id: existingRole.id },
          data: { isSystem: true },
        })
      : await prisma.role.create({
          data: {
            name,
            isSystem: true,
            description: `Base system role: ${name}`,
          },
        });
    rolesSeeded += 1;

    const rolesToSync = await prisma.role.findMany({
      where: name.startsWith('platform_')
        ? {
            name,
            OR: [
              { organizationId: null },
              { organization: { type: 'PLATFORM' } },
            ],
          }
        : { name },
    });
    const uniqueRoles = new Map(
      [systemRole, ...rolesToSync].map((role) => [role.id, role]),
    );

    for (const role of uniqueRoles.values()) {
      await syncRolePermissions(prisma, role.id, name);
    }
  }

  return { rolesSeeded, permissionsSeeded };
}

export async function syncRolePermissions(
  prisma: Pick<PrismaClient, 'permission' | 'rolePermission'>,
  roleId: string,
  roleName: (typeof BASE_ROLES)[number],
) {
  for (const permissionKey of ROLE_PERMISSIONS[roleName]) {
    const permission = await prisma.permission.findUniqueOrThrow({
      where: { key: permissionKey },
    });

    await prisma.rolePermission.upsert({
      where: {
        roleId_permissionId: {
          roleId,
          permissionId: permission.id,
        },
      },
      create: {
        roleId,
        permissionId: permission.id,
      },
      update: {},
    });
  }
}

if (require.main === module) {
  loadEnvironment();
  const connectionString = process.env.DATABASE_URL;
  if (!connectionString) throw new Error('DATABASE_URL is required.');
  const adapter = new PrismaPg({ connectionString });
  const prisma = new PrismaClient({ adapter });

  seedBaseRolesAndPermissions(prisma)
    .then((result) => {
      console.log(JSON.stringify(result));
    })
    .finally(async () => {
      await prisma.$disconnect();
    });
}
